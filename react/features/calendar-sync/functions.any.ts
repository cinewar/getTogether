...
function _isDisplayableCalendarEntry(entry: {
    ...
    return (
        ...
        !(
            (entry.allDay || entry.endDate - entry.startDate > ALLDAY_EVENT_LENGTH) &&
...
function _parseCalendarEntry(event: any, knownDomains: string[]) {
    if (event) {
        ...
        // no calendarId)
        if (isNaN(startDate) || isNaN(endDate) || (navigator.product !== 'ReactNative' && !url && !event.calendarId)) {
            // Ignore the event.
        } else {
            return {
                allDay: event.allDay,
...
/**
 * Retrieves a Get TogetherURL from an event if present.
 *
...
function _getURLFromEvent(
    event: { description: string; location: string; notes: string; title: string; url: string },
    knownDomains: string[]
) {
    const linkTerminatorPattern = '[^\\s<>$]';
    const urlRegExp = `http(s)?://(${knownDomains.join('|')})/${linkTerminatorPattern}+`;
    const schemeRegExp = `${APP_LINK_SCHEME}${linkTerminatorPattern}+`;
    const excludePattern = '/static/';
    const fieldsToSearch = [event.title, event.url, event.location, event.notes, event.description];

    for (const field of fieldsToSearch) {
        if (typeof field === 'string') {
            const match =
                _checkPattern(field, urlRegExp, excludePattern) || _checkPattern(field, schemeRegExp, excludePattern);
