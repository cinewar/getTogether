 ...
 * incompatible with Get Togetheron the client and/or server sides. The main
...
function _fixPathPart(pathPart?: string) {
    return pathPart ? pathPart.replace(new RegExp(_ROOM_EXCLUDE_PATTERN, 'g'), '') : pathPart;
}
...
function _objectToURLParamsArray(obj = {}) {
    ...
    for (const key in obj) {
        ...
        try {
            params.push(`${key}=${encodeURIComponent(JSON.stringify(obj[key as keyof typeof obj]))}`);
        } catch (e) {
...
export function parseStandardURIString(str: string) {
    ...
    // whitespace. Anyway, the Get Togetherapp is not known to utilize unencoded
...
/**
 * Parses a specific URI which (supposedly) references a Get Togetherresource
 * (location).
 *
 * @param {(string|undefined)} uri - The URI to parse which (supposedly)
 * references a Get Togetherresource (location).
 * @public
 * @returns {{
 *     contextRoot: string,
 *     hash: string,
...
export function parseURIString(uri?: string): any {
    ...
    // XXX While the components/segments of pathname are URI encoded, Get Together
    ...
    // Add the properties that are specific to a Get Togetherresource (location)
...
export function urlObjectToString(o: { [key: string]: any }): string | undefined {
    ...
    for (const urlPrefix of ['config', 'iceServers', 'interfaceConfig', 'devices', 'userInfo', 'appData']) {
        const urlParamsArray = _objectToURLParamsArray(
            o[`${urlPrefix}Overwrite`] || o[urlPrefix] || o[`${urlPrefix}Override`]
        );
