    ...
    MiddlewareRegistry.register((store) => (next) => (action) => {
        ...
        switch (type) {
            ...
            case RECORDING_SESSION_UPDATED: {
                const { error, id, initiator, liveStreamViewURL, mode, status, terminator, timestamp } =
                    action.sessionData;
...
function _registerForNativeEvents(store: IStore) {
    ...
    eventEmitter.addListener(ExternalAPI.SHOW_NOTIFICATION, ({ appearance, description, timeout, title, uid }: any) => {
 ...
 * {@link CONFERENCE_WILL_JOIN}. That is fine for the Get Togetherapp, because
