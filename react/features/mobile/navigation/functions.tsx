...
export function screenHeaderCloseButton(goBack: (e?: GestureResponderEvent | React.MouseEvent) => void) {
    ...
    if (Platform.OS === 'ios') {
        return <HeaderNavigationButton id={'close-screen-button'} label={t('dialog.close')} onPress={goBack} />;
    }

    return <HeaderNavigationButton id={'close-screen-button'} onPress={goBack} src={IconCloseLarge} />;
}

/**
 * Determines whether the {@code Prejoin page} is enabled by the app itself
 * (e.g. Programmatically via the Get TogetherSDK for Android and iOS).
 *
