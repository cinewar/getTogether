...
import WelcomePageTabs from '../../mobile/navigation/components/welcome/components/WelcomePageTabs';
...
class WelcomePage extends AbstractWelcomePage<IProps> {
    ...
    _doRenderInsecureRoomNameWarning() {
        return (
            <View style={[styles.messageContainer, styles.insecureRoomNameWarningContainer as ViewStyle]}>
                <Icon src={IconWarning} style={styles.insecureRoomNameWarningIcon} />
                <Text style={styles.insecureRoomNameWarningText}>{this.props.getUnsafeRoomTextFn(this.props.t)}</Text>
            </View>
        );
    }
    ...
    _onSettingsScreenFocused(focused: boolean) {
        ...
        Animated.timing(this.state.roomNameInputAnimation, {
            toValue: focused ? 0 : 1,
            duration: 500,
            useNativeDriver: true,
        }).start();
            ...
            return (
                <Animated.View style={this._getHintBoxStyle() as ViewStyle[]}>
                    <View style={styles.hintTextContainer as ViewStyle}>
                        <Text style={styles.hintText as TextStyle}>{t('welcomepage.roomnameHint')}</Text>
                    </View>
                    <View style={styles.hintButtonContainer as ViewStyle}>{this._renderJoinButton()}</View>
                </Animated.View>
            );
        }
    ...
    _renderJoinButton() {
        ...
        if (this.state.joining) {
            ...
            joinButton = (
                ...
                >
                    <View>
                        <LoadingIndicator color={BaseTheme.palette.icon01} size="small" />
                    </View>
                </TouchableHighlight>
            );
        } else {
            ...
            >
                <SafeAreaView style={styles.roomContainer as StyleProp<ViewStyle>}>
                    <View style={styles.joinControls}>
                        <Text style={styles.enterRoomText as StyleProp<TextStyle>}>{t('welcomepage.roomname')}</Text>
                        <Input
