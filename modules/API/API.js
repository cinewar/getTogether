...
import { getLocalVideoTrack, isLocalTrackMuted } from "../../react/features/base/tracks/functions";
import {
...
import { sendMessage, setPrivateMessageRecipient, toggleChat } from "../../react/features/chat/actions";
...
import { captureLargeVideoScreenshot, resizeLargeVideo } from "../../react/features/large-video/actions.web";
...
function initCommands() {
    commands = {
        ...
        "grant-recording-consent": (unmute) => {
            unmute ? APP.store.dispatch(grantRecordingConsentAndUnmute()) : APP.store.dispatch(grantRecordingConsent());
        },
        ...
        "set-follow-me": (value, recorderOnly) => {
            if (value) {
                sendAnalytics(
                    createApiEvent("follow.me.set", {
                        recorderOnly,
                    })
                );
            } else {
                sendAnalytics(
                    createApiEvent("follow.me.unset", {
                        recorderOnly,
                    })
                );
            }
        ...
        "toggle-moderation": (enabled, mediaType) => {
            ...
            const enable = mediaType === MEDIA_TYPE.VIDEO ? requestEnableVideoModeration : requestEnableAudioModeration;
            const disable =
                mediaType === MEDIA_TYPE.VIDEO ? requestDisableVideoModeration : requestDisableAudioModeration;
        ...
        "toggle-participants-pane": (enabled) => {
            const toggleParticipantsPane = enabled ? openParticipantsPane : closeParticipantsPane;
        ...
        "set-subtitles": (enabled, displaySubtitles, language) => {
            APP.store.dispatch(
                setRequestingSubtitles(enabled, displaySubtitles, language ? `translation-languages:${language}` : null)
            );
        },
        "toggle-tile-view": () => {
         ...
         * Only applies to certain Get Togetherdeploys.
    ...
    transport.on("request", (request, callback) => {
        ...
        switch (name) {
            ...
            case "capture-camera-picture": {
                ...
                APP.store.dispatch(openCameraCaptureDialog(callback, { cameraFacingMode, descriptionText, titleText }));
                break;
            }
...
function sanitizeMouseEvent(event) {
    const { clientX, clientY, movementX, movementY, offsetX, offsetY, pageX, pageY, x, y, screenX, screenY } = event;
 ...
 * interface to access Get Togetherfeatures by external applications that embed
 * Get Together.
 */
class API {
    _enabled;
     ...
     * information from external applications that embed Get Together. It also
     ...
     * @returns {void}
     */
    notifyReceivedChatMessage({ body, from, nick, privateMessage, ts } = {}) {
