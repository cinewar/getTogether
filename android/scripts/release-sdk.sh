...
echo "Releasing Get TogetherSDK ${SDK_VERSION}"
...
# Now build and publish the Get TogetherSDK and its dependencies
echo "Building and publishing the Get TogetherSDK"
pushd ${THIS_DIR}/../
./gradlew clean
...
if [[ $MVN_HTTP == 0 ]]; then
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Get TogetherSDK + dependencies: ${SDK_VERSION}"
    popd
fi
